...
import com.example.event.Adapter.BuyAgainAdapter
import com.example.event.Model.OrderDetails
import com.example.event.RecentOrderItems
import com.example.event.databinding.FragmentHistoryBinding
...
class HistoryFragment : Fragment() {
    ...
    private fun setDataInRecentBuyItem() {
        ...
        recentOrderItem?.let {
            with(binding) {
                buyAgainitemName.text = it.itemNames?.firstOrNull() ?: ""
                buyAgainitemPrice.text = it.itemPrices?.firstOrNull() ?: ""
                val image = it.itemImages?.firstOrNull() ?: ""
                Glide.with(requireContext()).load(image).into(buyAgainitemImage)
    ...
    private fun setPreviousBuyItemsRecyclerView() {
        ...
        for (i in 1 until listOfOrderItem.size) {
            listOfOrderItem[i].itemNames?.firstOrNull()?.let {
                buyAgainitemName.add(it)
                listOfOrderItem[i].itemPrices?.firstOrNull()?.let {
                    buyAgainitemPrice.add(it)
                    listOfOrderItem[i].itemImages?.firstOrNull()?.let {
                        buyAgainitemImage.add(it)
                    }
                }
            }
        }
